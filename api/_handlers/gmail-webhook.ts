...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  for (const msgId of messageIds.slice(0, 20)) {
    ...
    // Shadow log — best effort, never blocks the ACK
    try {
      await supabase
        .from('email_log_v3')
        .upsert(
          { user_id: profile.id, gmail_message_id: msgId, raw_payload: { historyId, emailAddress } },
          { onConflict: 'user_id,gmail_message_id', ignoreDuplicates: true }
        )
    } catch (err) {
      console.error('email_log_v3 insert failed (non-fatal):', err)
    }
  }
